...
@click.option('--network', type=click.Choice(list(ARCH_REGISTRY.keys())), default='SegDiffUnet', help='Network architecture')
@click.option('--loss', type=click.Choice(['pred_noise', 'pred_x0']), default='pred_noise', help='Loss function')
@click.option('--train-batch-size', default=8, help='Training batch size')
@click.option('--valid-batch-size', default=20, help='Validation batch size')
@click.option('--train-num-steps', default=50000, help='Number of training steps')
@click.option('--valid-every', default=500, help='Validate every N steps')
@click.option('--save-every', default=1000, help='Save model every N steps')
...
def main(dset, network, loss, train_batch_size, valid_batch_size, train_num_steps, valid_every, save_every, interactive):
    if interactive:
        ...
        loss = click.prompt('Loss function', type=click.Choice(['pred_noise', 'pred_x0']), default=loss)
    ...
    print('Saving the result to the folder:', model_folder_name)
